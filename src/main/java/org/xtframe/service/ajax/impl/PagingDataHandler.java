...
/**
 * @ClassName: PagingDataHandler 
 * @Description: 执行数据库分页处理
 * @author yong.sun
 * @date 2013-9-23
 */
@Service
@Scope("prototype")
public class PagingDataHandler extends AbstractBasicExecute implements IAjaxDataHandler {
	...
	public Map<String, Object> execute(String strData) {
		...
		try {
			...
			long offset = Long.valueOf(StringUtil.toString(objJson.get("offset"), "0"));
			int maxsize = Integer.valueOf(StringUtil.toString(objJson.get("maxsize"), ""));
			...
			String totalSql = "select count(*) from (" + sqlCheck.getSqlData().getSql() + ") t";
			String querySql = dataBase.getDBPagineSql(sqlCheck.getSqlData().getSql(), offset, maxsize);

			long total = dataBase.findRows(totalSql, sqlCheck.getSqlData().getPreparedParameters());
			List<Map<String, Object>> query = dataBase.executeQuery(querySql, sqlCheck.getSqlData().getPreparedParameters());
			map.put(STATUS, true);
