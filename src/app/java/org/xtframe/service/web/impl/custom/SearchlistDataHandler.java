...
/**
 * @ClassName: SearchlistDataHandler 
 * @Description: 分类进入处理
 * @author wen.xu
 * @date 2015-11-20
 */
public class SearchlistDataHandler extends AbstractBasicExecute implements IWebDataHandler {
	...
	public Object execute(Map<String, Object> parame, Map<String, Object> data, WebPageDefined wpd) {
		...
		try {
			String cls ="";
			String conidtion = "";
			if(parame.containsKey("cls")){
				cls= parame.get("cls").toString();
				conidtion = "cls:"+cls+"*";				
			}else{
				cls = "*";
				conidtion = "cls:"+cls;
			}
			int start = Integer.parseInt(parame.get("start").toString());
			int size=Integer.parseInt(parame.get("size").toString());
