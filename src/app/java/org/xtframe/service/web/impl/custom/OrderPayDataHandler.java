...
/**
 * @ClassName: OrderPayDataHandler 
 * @Description: 订单支付处理
 * @author yong.sun
 * @date 2013-11-1
 */
public class OrderPayDataHandler extends AbstractBasicExecute implements IWebDataHandler {
	...
	public Object execute(Map<String, Object> parame, Map<String, Object> data, WebPageDefined wpd) {
		try {
			...
			List<Map<String, Object>> orderslist = (List<Map<String, Object>>)data.get("order");
			Map<String, Object> order = orderslist.get(0);
			...
			packageParams.put("body", details.get("goods_name").toString());								//商品描述   
			...
			packageParams.put("out_trade_no", order.get("orders_id").toString());		//商户订单号  
			packageParams.put("spbill_create_ip", CommonUtil.getIpAddr(request));		//订单生成的机器IP，指用户浏览器端IP(用户的公网IP)
			packageParams.put("total_fee", ""+(int)(Float.parseFloat(order.get("total_price").toString()) * 100));		//商品总金额,以分为单位
