		...
		columns = [
		    {
				...
				renderer : function(value,record){
					if(value == '1'){
						return '<span class="label label-success">满'+record.max_money+'包邮</span>';
					}else if(value == '2'){
						return '<span class="label label-success">'+record.min_money+'到'+record.max_money+'包邮</span>';
					}else if(value == '3'){
						return '<span class="label label-success">按快递公司标准计费</span>';
					}else if(value == '4'){
						return '<span class="label label-success">固定收费</span>';
					}
				}
			},{
				title : '最小金额',
				...
				renderer : function(value,record){
					if(value == '0'){
						return '<span class="label label-info">暂未实施</span>';
					}else{
						return '<span class="label label-success">正在实施</span>';
					}
				}
			}],
		params = {
						...
						items : [{
							...
							handler : function() {
								...
								PubOPT.openPage({
									id: pgid,
									title: '配送信息添加',
									href: contextPath + '/p100115.html?ppid=' + PubOPT.getCurrentPageId()
								});
							}
						}, {
							btnCls : 'button button-small',
							...
							handler : function() {
								...
								PubOPT.openPage({
									id: pgid,
									title: '配送信息编辑',
									href: contextPath + '/p100115.html?ppid=' + PubOPT.getCurrentPageId() + '&code=' + model.id 
								});
							}
						}, {
							btnCls : 'button button-small',
