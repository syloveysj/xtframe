...
<html lang="en">
	<head>
		...
		<script type="text/javascript" src="${model._contextPath}/mobile/resources/js/sellingList.js"></script>
		<script type="text/javascript" src="${model._contextPath}/web/js/pull-to-refresh/iscroll.js"></script>
	</head>
	<body id="totop">
		<!--头部--> 
		<div class="header">
			<a href="javascript:window.history.back();" class="tbr_rbtn" data-icon="&#xe62a;" title="返回" style="width:auto">推荐畅销</a>
	...
	<!--内容-->
	<input type="hidden" id="totalcount" value="${model.sellingCx[0].totalcount}"/>
	<input type="hidden" id="start" value="${model.sellingCx[0].start}"/>
  	<input type="hidden" id="pageSize" value="6"/>
  	<input type="hidden" id="contextPath" value="${model._contextPath}"/>
<#if model.sellingList?exists && model.sellingList?has_content && (model.sellingList?size>0) >
		<div id="wrapper">
		<div id="scroller">
		<!--author--> 
		<div class="bookkinds" >
		<div style="height:46px">&nbsp;</div>
			...
			<div class="fav_list">
			<ul id="thelist">
				<#list model.sellingList as ls>
					<li  style="height:auto">
						<a href="${model._contextPath}/p100050.html?id=${ls.goods_id}" >
		                	<img src="${model._contextPath}${ls.small_relative_path}" style="border-radius:0">
							<div class="txtall">
								<h4 class="titleblue"><#if ls.goods_name?has_content && (ls.goods_name?length>16)>${ls.goods_name[0..16]}..<#elseif ls.goods_name?has_content && (ls.goods_name?length<=16)>${ls.goods_name}</#if></h4>
