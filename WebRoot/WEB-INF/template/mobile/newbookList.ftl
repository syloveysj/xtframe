...
<html lang="en">
	<head>
		...
		<script type="text/javascript" src="${model._contextPath}/mobile/resources/js/newbookList.js"></script>
		<script type="text/javascript" src="${model._contextPath}/web/js/pull-to-refresh/iscroll.js"></script>
	</head>
	<body id="totop">
		<!--头部--> 
		<div class="header">
			<a href="javascript:window.history.back();" class="tbr_rbtn" data-icon="&#xe62a;" title="返回" style="width:auto">最近新书</a>
	...
	<!--内容-->
	<input type="hidden" id="totalcount" value="${model.newbookZx[0].totalcount}"/>
	<input type="hidden" id="start" value="${model.newbookZx[0].start}"/>
  	<input type="hidden" id="pageSize" value="6"/>
  	<input type="hidden" id="contextPath" value="${model._contextPath}"/>
<#if model.newbookList?exists && model.newbookList?has_content && (model.newbookList?size>0) >
		<div id="wrapper">
		<div id="scroller">
		<!--author--> 
		<div class="bookkinds" >
