...
function pullUpAction () {  //自定义执行数据查询方法
	setTimeout(function () {
		...
		pageSize = 5; 
		var data = AjaxUtil.command.executeQuery("wx_author_list_2",{start:next,size:pageSize}); //查询书籍
		for(var i=0;i<data.rows.length;i++){
			var row = data.rows[i];
			$("#thelist").append("<li>" +
					"<a href='"+contextPath+"/p100066.html?id="+row.author_id+"'>" +
					"<img src='"+contextPath+row.small_relative_path+"'"+
					"<p class='atrname'>"+row.author_name+"&nbsp;&nbsp;"+
					"<span style='font-size:14px;'>"+substrDemo(row.author_detail,0,16)+"</span></p>"+
					"<p class='atrtxt'><span>代表作品&nbsp;</span>"+
					substrDemo(row.magnum,0,16)+"</p></a></li>"); // 页面ui元素追加子元素li
		}
