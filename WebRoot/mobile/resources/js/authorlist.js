...
function pullUpAction () {  //自定义执行数据查询方法
	setTimeout(function () {
		...
		pageSize = 5; 
		var data = AjaxUtil.command.executeQuery("wx_authorlist_2",{start:next,size:pageSize,id:$('#id').val()}); //查询书籍
		for(var i=0;i<data.rows.length;i++){
			...
			$("#thelist").append(
					'<li><a href="'+contextPath+'/p100050.html?id='+row.goods_id+'">'
						+'<div class="img_wrap"><img src="'+contextPath+row.small_relative_path+'" width="80"></div> ' 
						+'<div class="text_wrap">' 
							+'<p class="name">'+goods_name+'</p>'
							+'<p class="promo">'+goods_author+' 著</p>'
							+'<p class="price"><span class="feed">'+goods_factory+' 出版.</span><span class="y_pf">¥ '+goods_price+'</span></p>' 
						+'</div>'
					+'</a></li>'); // 页面ui元素追加子元素li
