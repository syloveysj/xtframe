...
function initPage(){
	...
	if(cpage==totalpage){ //判断当前页是否是最后一页
		upflag=true;
		document.getElementById('pullUp').querySelector('.pullUpLabel').innerHTML = '无更多畅销书籍'; 
	}else{
		document.getElementById('pullUp').querySelector('.pullUpLabel').innerHTML = '上拉刷新更多畅销书籍...';
	}
	pagePullWidth();
}
...
function pullUpAction () {  //自定义执行数据查询方法
	setTimeout(function () {
		...
		pageSize = 5; 
		var data = AjaxUtil.command.executeQuery("wx_selling_1",{start:next,size:pageSize}); //查询书籍
