...
function initPage(){
	...
	if(cpage==totalpage){ //判断当前页是否是最后一页
		upflag=true;
		document.getElementById('pullUp').querySelector('.pullUpLabel').innerHTML = '无更多特卖书籍'; 
	}else{
		document.getElementById('pullUp').querySelector('.pullUpLabel').innerHTML = '上拉刷新更多特卖书籍...';
	}
	pagePullWidth();
}
...
function pullUpAction () {  //自定义执行数据查询方法
	setTimeout(function () {
		...
		pageSize = 5; 
		var data = AjaxUtil.command.executeQuery("wx_tuanbookwz_1",{start:next,size:pageSize}); //查询书籍
...
//			var small_relative_path =  row.small_relative_path;
			...
			$("#thelist").append(
					...
					'<div class="txtall">'+
					'<h4 class="titlegreen">'+goods_name+'</h4>'+
